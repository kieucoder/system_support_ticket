/* ==============================================================
   FILE: wwwroot/assets/js/forgot-password-step2.js
   AUTHOR: Senior UI/UX Designer & Frontend Developer
   DESCRIPTION: Handles static forgot password flow (Step 2).
   ============================================================== */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!step2Form || !btnResetPassword || !btnResendOtp) {
        return;
    }

    // Load identifier from localStorage
    const savedIdentifier = localStorage.getItem('resetIdentifier') || 'example@techsupport.vn';
    ...
    if (displayTargetText) {
        displayTargetText.textContent = maskIdentifier(savedIdentifier);
    }
    ...
    const updateTimerUi = () => {
        ...
        if (otpExpiryDisplay) {
            otpExpiryDisplay.textContent = formatted;
        }
        ...
        if (expirySeconds <= 0) {
            ...
            showToast('Mã OTP đã hết hạn. Vui lòng bấm gửi lại mã.', 'danger');
        } else {
    ...
    step2Form.addEventListener('submit', (e) => {
        ...
        setTimeout(() => {
            ...
            if (timerHandle) clearInterval(timerHandle);
            localStorage.removeItem('resetIdentifier'); // clear cache

            // Redirect to Step 3 success page
            window.location.href = 'step3-verify-account.html';
        }, 1800);
    });
    ...
    // Auto start timers and focus first box
    startTimer();
