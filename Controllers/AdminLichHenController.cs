...
public class AdminLichHenController : Controller
{
    ...
    /// <summary>
    /// 2. XEM CHI TIẾT LỊCH HẸN (Details)
    /// </summary>
    ...
    public async Task<IActionResult> Details(int id)
    {
        ...
        var (statusTitle, badgeClass) = GetStatusInfo(lichHen.TrangThai);
    ...
    public async Task<IActionResult> Edit(int id, AdminEditLichHenViewModel model)
    {
        ...
        // Kiểm tra xung đột lịch làm việc nếu có chọn KTV
        if (model.IdNhanVien.HasValue && model.IdNhanVien.Value > 0)
        {
            ...
            {
                ModelState.AddModelError("", $"Xung đột lịch làm việc: {availResult.Message} ({string.Join("; ", availResult.Reasons)})");
                model.NhanVienList = await _context.NhanViens
                    .AsNoTracking()
                    .OrderBy(n => n.HoTen)
                    .Select(n => new SelectListItem
    ...
    /// <summary>
    /// 6. HỦY LỊCH HẸN ĐÃ XÁC NHẬN (Cancel - POST)
    /// </summary>
    [HttpPost("Cancel")]
    [ValidateAntiForgeryToken]
    ...
    private static AdminLichHenItemViewModel MapToItemViewModel(LichHen l)
    {
        ...
        return new AdminLichHenItemViewModel
        {
            ...
            GhiChu = l.GhiChu,
            TrangThaiCode = l.TrangThai ?? "ChoXacNhan",
    ...
    {
        if (string.IsNullOrEmpty(status)) return ("Chờ xác nhận", "bg-warning text-dark");

        return status.Trim() switch
        {
            "ChoXacNhan" or "Chờ xác nhận" => ("Chờ xác nhận", "bg-warning text-dark"),
